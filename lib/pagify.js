...
export default async function pagifier() {
  // const handler = {
  //   spanStart: '<span class="pagy"',
  //   lastSpanStart: '<span class="pagy last-pagy"',
  //   spanEnd: '</span>',
  //   hrHtml: '<hr class="section">',
  //   book: {},
  //   index: {},
  //   HREF_REGEX: /<a href=([^>]*?)>/g,
  //   ANCHOR_HREF_REGEX: /<a href=/g,
  //   ANCHOR_REGEX: /<a\s+(?:[^>]*?\s+)?href=(["'])(.*?)\1([^>]*)?>(.*?)<\/a>/g,
  //   NEWLINE_REGEX: /\n/g,
  //   NEWLINE_SPACE_REGEX: /\n /g,
  //   ANCHOR_HREF_REV_REGEX: /<ahref=/g,
  // };
  ...
  const bookLayoutTemplate = await fse
    ...
    .catch((err) => iconsole.error(chalk.red('Could not read style.css from the layout /templates', err)));

  const bookHeadTemplate = await fse
    .readFile(path.join('.', 'templates', 'head.html'))
    .catch((err) => iconsole.error(chalk.red('Could not read head.html from templates', err)));
  ...
  */
  ...
  function adjustInlineTags(selectedHtml, content) {
    ...
    let newSelectedHtml = selectedHtml;

    if (lastIndexEm !== -1 && lastIndexEm > lastIndexEmEnd) {
      newContent = `<em>${content}`;
      newSelectedHtml += '</em>';
    }
  ...
  for (let i = 0; i < prebook.length; i += 1) {
    ...
    // ^^^ this is the HTML that is pageHtml + new tag html
    // and will be evaluated with puppeteer
    let htmlToAdd = '';
    ...
    switch (tag) {
      ...
      case 'hr':
        ...
        if (pageHtml !== '') { pageHtml += hrHtml; } else if (book[`page-${pageCounter - 1}`]) {
          // If the chapter's content has already been included on last page, add the
          // horizontal rule on the last page
      ...
      case 'h4':
        ...
        break;
      default:
        iconsole.log(`Unhandled tag encountered = ${tag}`);
    }
    prevTag = tag;
  }
