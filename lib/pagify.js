    ...
    if(chapterStartPara) chapterStartPara = !chapterStartPara
        ...
        if(imgSrc.startsWith("http")) {
          ...
          const imgNeedsWidth = $currentSpan.width()
				...
				if (nextPage) {
					imageToAddNext = true
					imageHtmlToAdd = `<img class="overlay center" width = "${imageAttr.width}%" src = "${elem.innerHtml}" />`
				} else {
					pageHtml += `<img class="overlay center" width = "${imageAttr.width}%" src = "${elem.innerHtml}" />`
				}
