...
export default async function pagifier() {
  ...
  const pageTemplateHtml = await fse.readFile(path.join(__dirname, '..', 'templates', 'pagy-template.html'), 'utf-8').catch((err) => { process.stderr(`Could not read the page-template file: ${err}`); });

  const bookrc = await fse.readJson(path.join('.', '.bookrc')).catch((err) => {
    if (err) process.stderr(chalk.red('Could not read .bookrc ', err));
  });
  ...
  const prebook = await fse.readJson(path.join('.', 'interim', 'tmp', '.prebook')).catch((err) => {
    if (err) process.stderr(chalk.red('Could not read .book json', err));
  });

  const bookLayoutTemplate = await fse.readFile(path.join('.', 'templates', 'style.css')).catch((err) => {
    if (err) process.stderr(chalk.red('Could not read style.css from templates', err));
  });
  const bookHeadTemplate = await fse.readFile(path.join('.', 'templates', 'head.html')).catch((err) => {
    if (err) process.stderr(chalk.red('Could not read head.html from templates', err));
  });
  ...
  for (const elem of prebook) {
    ...
    switch (tag) {
      ...
      default:
        process.stderr(`Unhandled tag encountered = ${tag}`);
    }
    prevTag = tag;
  }
  ...
  await fse.writeFile(path.join('.', 'interim', 'tmp', '.book'), JSON.stringify(book, null, 2)).catch((err) => {
    if (err) process.stderr(chalk.bold.red('Failed to write .book json', err));
  });
  process.stdout(chalk.green(`Pagification… (.book) is ${chalk.blue('complete')}`));

  await fse.writeFile(path.join('.', 'interim', 'tmp', '.index'), JSON.stringify(index, null, 2)).catch((err) => {
    if (err) process.stderr(chalk.bold.red('Failed to write index json', err));
  });
  process.stdout(chalk.green(`A book.index was ${chalk.blue('prepared.')}`));
