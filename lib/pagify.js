...
export default async function pagifier() {
  ...
  */
  ...
  function getSpannedHtmlOfHtml(paraHtml) {
  ...
  for (let i = 0; i < prebook.length; i += 1) {
    ...
    switch (tag) {
      ...
      case 'blockquote':
        ...
        // eslint-disable-next-line max-len
        pageHtmlToCheck = pageHtml + getIndentAndStretchParaHtml(tag, getSpannedHtmlOfHtml(pContent), false, false, chapterStart);
        do {
          ...
          if (hiddenSpan) {
            if (hiddenSpan !== 0) {
              ...
              pageHtmlToCheck = getIndentAndStretchParaHtml(tag, getSpannedHtmlOfHtml(pContent), true, false);
              paraContinued = true;
            } else {
              pageHtmlToCheck = getIndentAndStretchParaHtml(tag, getSpannedHtmlOfHtml(pContent), paraContinued, false, chapterStart);
            }
            finishPage(false);
          } else {
            pageHtml += htmlToAdd;
      ...
      case 'ul':
        ...
        for (let j = 0; j < list.length; j += 1) {
          ...
          pageHtmlToCheck = `${pageHtml}${tagHtml}<li>${getSpannedHtmlOfHtml(liContent)}</li></${tag}>`;
          ...
          do {
            ...
            if (hiddenSpan) {
              if (hiddenSpan !== 0) {
                ...
                pageHtmlToCheck = getSelectedSplitAndStretchListHtml(tag, currentliIndex, getSpannedHtmlOfHtml(liContent), true, false, true, true);

                liContinued = true;
              } else {
                pageHtmlToCheck = getSelectedSplitAndStretchListHtml(tag, currentliIndex, getSpannedHtmlOfHtml(liContent), false, false, true, true);
                pageHtml += `</${tag}>`;
              }
              finishPage(false);
