async function pagify() {
  ...
  const startPage = bookrc.start_page ? parseInt(bookrc.start_page) : 9
  ...
  let imageToAddNext = false, imageHtmlToAdd = '', pageHtmlToCheck = '';
  ...
  for (const elem of prebook) {
    ...
    switch (tag) {
      ...
      case 'h3': case 'h4':
        finishPage(true)
        pageHtml = `<${tag}>${elem.innerHtml}</${tag}>`;
        index[indexer++] = `<li><a class = "page" h2ef="${pageCounter}">${elem.innerHtml}</a> <span class="flex">${pageCounter}</span> </li>`;
