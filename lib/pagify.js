function pagify() {
	...
	let index = {};
	// let page_count = 1;
	const emptyPageHtml = fse.readFileSync(path.join(__dirname, '..', 'templates', 'pagy-template.html'), 'utf-8');

	fse
		.readJson(path.join('.', '.bookrc')).then(bookrc => {
			return parseInt(bookrc.start_page)
		}).then((page_count) => {
			fse.readJson(path.join('.', 'interim', 'tmp', '.prebook'))
				.then(prebook => {
		...
		}).catch(err => console.log('Couldn\'t read start_page from .bookrc', err))
