...
export default async function pagifier() {
  ...
  const prebook = await fse
    ...
    .catch((err) => iconsole.error(chalk.red('Could not read .prebook json', err)));

  const bookLayoutTemplate = await fse
    .readFile(path.join('.', 'templates', 'style.css'))
    .catch((err) => iconsole.error(chalk.red('Could not read style.css from templates', err)));
  ...
  for (let i = 0; i < prebook.length; i += 1) {
    ...
    // and will be checked with puppeteer
    let htmlToAdd = '';
