  ...
  const page = (await browser.pages())[0];
    ...
    if(chapterStartPara) chapterStartPara = !chapterStartPara
        ...
        if(imgSrc.startsWith("http")) {
          let paths = imgSrc.split("/")
          imgSrc = "data:image/jpeg;base64,"+fse.readFileSync(path.resolve('/Users/sa/Projects/ror/bubblin.io/guten/the-iliad' , 'assets', 'images', paths[paths.length-1])).toString('base64')
        }
        
				pageHtmlToCheck = pageHtml + `<img id="checkImg" src = "${imgSrc}" />`
          ...
          debugger
					...
					if (heightOverFlowing) {
						let availableHeight = pageDivEl.clientHeight - pageDivEl.offsetTop
					...
					widthPercent = Math.ceil(($currentSpan.width() * 100) / $pageDiv.width())
				...
				if (nextPage) {
					imageToAddNext = true
					imageHtmlToAdd = `<img width = "${imageAttr.width}%" src = "${elem.innerHtml}" />`
				} else {
					pageHtml += `<img width = "${imageAttr.width}%" src = "${elem.innerHtml}" />`
				}
