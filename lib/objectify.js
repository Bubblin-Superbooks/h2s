      ...
      $('body').children().each((i, elem) => {
        ...
        switch (key) {
          ...
          case 'ol': case 'ul':
            ...
            break;
          case 'br':
            console.log('Insert a line break here? Decision taken on March 10th, 2023: No.');
            break;
          default:
            console.log('We have an unhandled HTML tag: ', key, i);
        }
      ...
      fse.mkdirs(path.join('interim', 'tmp'))
        .then(() => {
          fse.writeFile(path.join('.', 'interim', 'tmp', '.prebook'), JSON.stringify(book, null, 2))
            .then(() => {
              console.log(chalk.cyanBright('Prebook object saved.'));
            }).catch((err) => {
              if (err) { console.log(chalk.bold.red('Failed to create `interim/tmp/.prebook`: ', err)); }
            });
        })
        .catch((err) => {
          if (err) { console.log(chalk.bold.red('Failed to create /tmp directory: ', err)); }
        });
    }).catch((err) => {
      if (err) { console.log(chalk.bold.red('Failed to pick up the contents of sanitized html', err)); }
    });
}
