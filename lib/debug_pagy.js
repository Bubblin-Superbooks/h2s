...
fsp
	...
	.then((prebook) => {
		...
		return puppeteer.launch({ headless: false }).then(async (browser) => {
			...
			await page.setViewport({ width: 462, height: 600 }); // Do not change these values unless you know what it entails.
								...
								do {
									...
									if (hiddenSpan) {
										if (hiddenSpan != 0) {
											...
											if (paraContinued) html += `<p class="no-indent">${selectedParaHtml}</p>`;
