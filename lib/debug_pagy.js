...
import { fileURLToPath } from 'url';
  ...
  const NEWLINE_SPACE_REGEX = new RegExp(/\n /g)
		export default async function debugpagifier() {
  ...
  const __dirname = path.dirname(__filename);
	...
	const pageTemplateHtml =  fse.readFileSync(path.join(__dirname, '..', 'templates', 'pagy-template.html'), 'utf-8')
	//const bookrc =  fse.readJsonSync(path.join('.', '.bookrc'))
	const startPage =  9

	//const prebook =  fse.readJsonSync(path.join('.', 'templates', 'test.prebook'))


	const bookLayoutTemplate =  fse.readFileSync(path.join('.', 'templates', 'style.css'))

	const bookHeadTemplate =  fse.readFileSync(path.join('.', 'templates', 'head.html'))
	...
	let tagAdded = false,
		tagHtml = ''; // the flag is to check whether ol/ul tag needs to be added for this li

		fse
	.readJson(path.join('.', 'templates', 'test.prebook'))
	.then((prebook) => {
  ...
  const page = (await browser.pages())[0];
	...
	let imageToAddNext = false,
		imageHtmlToAdd = '',
		pageHtmlToCheck = '',
    chapterStartPara = false,
    prevTag = '', htmltoAdd = '',tag;
    ...
    if(chapterStartPara) chapterStartPara = !chapterStartPara
      ...
      case 'div':
        ...
        const hiddenFlag = await page.evaluate(() => {
          ...
          let $currentDiv = $("#hiddenDivCheck")
          const visibleOffsetHeight = pageDivEl.offsetTop + pageDivEl.clientHeight
          let divNeedsHeight = $currentDiv.offset().top + $currentDiv.height()
          let heightOverFlowing = divNeedsHeight > visibleOffsetHeight
        ...
        if(imgSrc.startsWith("http")) {
          let imagePath = imgSrc.split("/")
          imgSrc = "data:image/jpeg;base64,"+fse.readFileSync(path.resolve('.','templates', 'assets', 'images', imagePath[imagePath.length-1])).toString('base64')
        }
				...
				const imageAttr = await page.evaluate(() => {
					...
					let ratio = $currentSpan.width() / $currentSpan.height()
          ...
          const imgNeedsWidth = $currentSpan.width()
					...
					let isHeightLessThanHalfPage = ($currentSpan.offset().top - pageDivEl.offsetTop) > pageDivEl.clientHeight / 2
					...
					if (heightOverFlowing) {
						let availableHeight = pageDivEl.clientHeight - $currentSpan.offset().top + pageDivEl.offsetTop
						$currentSpan.height(availableHeight)
						$currentSpan.width(availableHeight * ratio)
					}

					let lineHeight = parseInt($pageDiv.css('line-height').replace('px', ''))
					let multiple = Math.floor($currentSpan.height() / lineHeight)
	...
	await fse.writeFile(path.join( '.book'), JSON.stringify(book, null, 2)).catch(err => {
    ...
    if (lastIndexStrong != -1 && lastIndexStrong > lastIndexStrongEnd) {
      ...
      let href = HREF_REGEX.exec(anchorString)
