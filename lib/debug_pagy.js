    ...
    //fsp.readJson(path.join('.', 'interim', 'tmp', '.prebook')).then((prebook) => {
		...
		return puppeteer.launch({headless:false}).then(async browser => {
			...
			await page.setViewport({ width: 472, height: 613 })
			...
			for (const key in prebook) {
				if (prebook.hasOwnProperty(key)) {
                  ...
                  await page.waitForSelector(".last-pagy");
                      ...
                      checkForTagBreaks(selectedParaHtml, pContent)
                      html += `<p>${selectedParaHtml}</p>`
	...
	function getSpannedHtmlOfPara(paraHtml) {
        ...
        if(i == lastIndex)
          spannedHtml = `${spannedHtml}${lastSpanStart} id="${i}"> ${v}${spanEnd}`
        else
          spannedHtml = `${spannedHtml}${spanStart} id="${i}"> ${v}${spanEnd}`
      }
    ...
    function checkForTagBreaks(selectedParaHtml, pContent) {
      ...
      if(lastIndexStrong != -1 && lastIndexStrong > lastIndexStrongEnd) {
        selectedParaHtml += selectedParaHtml + '</strong>'
        pContent = '<strong>' + pContent
      }

      return selectedParaHtml
    }
